...
TEST_CASE("Sphere Collision Detection", "[detect]") {
	SphereShape* sphere = new SphereShape(1);
	CHECK_FALSE(sphere->collides(sphere, translate(vec3f(1, 1, 1)), translate(vec3f(-1, -1, -1))));
	...
	CHECK(sphere->collides(sphere, translate(vec3f(0, -1, 0))));
	CHECK(sphere->collides(sphere, translate(vec3f(0, 0, -1))));
	CHECK_FALSE(sphere->collides(sphere, translate(vec3f(0.0, 0.0, 1.5)) * scale(vec3f(0.4, 0.4, 0.4))));
	CHECK_FALSE(sphere->collides(sphere, translate(vec3f(0.0, 0.0, 1.5)), scale(vec3f(0.4, 0.4, 0.4))));
	delete sphere;
}
TEST_CASE("Cylinder Collision Detection", "[detect]") {
	...
	CHECK_FALSE(cylinder->collides(cylinder, translate(vec3f(1, 1, 1)), translate(vec3f(-1, -1, -1))));
	...
	CHECK(cylinder->collides(cylinder, translate(vec3f(0, -1, 0))));
	CHECK(cylinder->collides(cylinder, translate(vec3f(0, 0, -1))));
	CHECK_FALSE(cylinder->collides(cylinder, translate(vec3f(0.0, 0.0, 1.5)) * scale(vec3f(0.4, 0.4, 0.4))));
	CHECK_FALSE(cylinder->collides(cylinder, translate(vec3f(0.0, 0.0, 1.5)), scale(vec3f(0.4, 0.4, 0.4))));
	delete cylinder;
}
TEST_CASE("AABB Collision Detection", "[detect]") {
	...
	CHECK_FALSE(aab1->collides(aab1, translate(vec3f(1, 1, 1)), translate(vec3f(-1, -1, -1))));
	...
	CHECK(aab1->collides(aab1, translate(vec3f(0, -1, 0))));
	CHECK(aab1->collides(aab1, translate(vec3f(0, 0, -1))));
	CHECK_FALSE(aab1->collides(aab1, translate(vec3f(0.0, 0.0, 1.5)) * scale(vec3f(0.4, 0.4, 0.4))));
	CHECK_FALSE(aab1->collides(aab1, translate(vec3f(0.0, 0.0, 1.5)), scale(vec3f(0.4, 0.4, 0.4))));
...
TEST_CASE("Transform Collision Detection", "[detect]") {
	...
	CHECK_FALSE(cylinder->collides(cylinder, rotate(vec3f(0.00, 1.57, 0.00)), translate(vec3f(0.00, 1.51, 0.00))));
	delete cylinder;
}
...
void test_mtv(BoundingShape* shape, vec3f dx, vec4f expected, float shiftEpsilon = 0.001f, vec4f tolerance = vec4f(0.01f, 0.01f, 0.01f, 0.001f))
{
	...
	CHECK_FALSE(shape->collides(shape, translate(dx + vec3f(mtv) * (mtv.w + shiftEpsilon))));
}
void test_mtv(BoundingShape* shape0, BoundingShape* shape1, vec3f dx, vec4f expected, float shiftEpsilon = 0.001f, vec4f tolerance = vec4f(0.01f, 0.01f, 0.01f, 0.001f))
{
	...
	CHECK_THAT(mtv.y, WithinAbs(expected.y, tolerance.y));
	CHECK_THAT(mtv.z, WithinAbs(expected.z, tolerance.z));
	CHECK_FALSE(shape0->collides(shape1, translate(dx + vec3f(mtv) * (mtv.w + shiftEpsilon))));
}
TEST_CASE("Trivial Collision MTV", "[mtv]") {
	...
	REQUIRE_FALSE(sphere->collides(sphere, translate(vec3f(1, 0, 0) + vec3f(mtv) * (mtv.w + 0.001f))));
	delete sphere;
}
TEST_CASE("Cylinder Collision MTV", "[mtv]") {
	CylinderShape* cylinder = new CylinderShape(2.0, 0.5);
	test_mtv(cylinder, vec3f(0.9, 0.0, 0.0), vec4f(1.0, 0.0, 0.0, 0.1), 0.001f, vec4f(0.02, 0.02, 0.02, 0.001));
	test_mtv(cylinder, vec3f(0.6, 0.0, 0.6), vec4f(0.707, 0.0, 0.707, 0.1515), 0.001f);
	test_mtv(cylinder, vec3f(0.6, 0.3, 0.6), vec4f(0.707, 0.0, 0.707, 0.1515), 0.001f);
...
TEST_CASE("Mixed Collision MTV", "[mtv]") {
	...
	test_mtv(cylinder, aab, vec3f(1.4, 0.0, 0.0), vec4f(1.0, 0.0, 0.0, 0.1), 0.001f);
	test_mtv(cylinder, aab, vec3f(1.3, 0.0, 1.2), vec4f(0.827, 0.0, 0.561, 0.139), 0.001f);
