...
Environment* testEnvironment() {
  ...
  */
  ...
  environment->addConvex({vec3f(-5, 0, 0), vec3f(0, 0, -5), vec3f(-5, 0, -5),
                          vec3f(-5, 5, 0), vec3f(0, 5, -5), vec3f(-5, 5, -5)});
  environment->addConvex({vec3f(-5, 0, 0), vec3f(0, 0, 5), vec3f(-5, 0, 5),
                          vec3f(-5, 5, 0), vec3f(0, 5, 5), vec3f(-5, 5, 5)});
  environment->addBox(vec3f(0, 0, -6), vec3f(5, 5, -5));
  environment->addBox(vec3f(0, 0, 5), vec3f(5, 5, 6));

  environment->addConvex({vec3f(5, 0, -5), vec3f(10, 0, -5), vec3f(10, 3, -5),
                          vec3f(5, 0, 5), vec3f(10, 0, 5), vec3f(10, 3, 5)});
...
void arrivesAt(vec3f pos, PObject* obj, size_t maxTicks = 1000,
               float epsilon = 0.25f) {
  size_t ticks = 0;
  do {
    level->tick();
    ...
    if (ticks > maxTicks) {
      ...
      INFO("Reached (" + std::to_string(obj->getPos().x) + ", " +
           std::to_string(obj->getPos().y) + ", " +
           std::to_string(obj->getPos().z) + ")");
      CHECK(false);
      return;
    }
...
void playerArrivesAt(std::initializer_list<vec3f> poss,
                     std::initializer_list<vec3f> vels, Player* player,
                     ControlModifierData* cmd, size_t maxTicksPerStep = 1000,
                     float epsilon = 0.25f) {
  ...
  for (int i = 0; i < poss.size(); i++) {
    ...
    do {
      ...
      if (ticks > maxTicksPerStep) {
        INFO("Test exceeded maximum number of ticks at step " +
             std::to_string(i));
        INFO("Reached (" + std::to_string(player->getPos().x) + ", " +
...
TEST_CASE("Control, Collision, Slopes", "[game]") {
  ...
  player->setPos(vec3f(-4.0, 0.0, 0.0));
  player->vel = vec3f(0, 0, 0);
  playerArrivesAt({vec3f(0.0, 0.0, -4.0), vec3f(0.0, 0.0, 4.5)},
                  {vec3f(0.1, 0.0, -0.1), vec3f(-0.01, 0.0, 0.15)}, player,
                  cmd);
...
TEST_CASE("Jump", "[game]") {
  ...
  player->vel = vec3f(0, 0, 0);
  cmd->doJump = true;
  playerArrivesAt({vec3f(4.5, 4.0, 0.0), vec3f(-4.5, 0.0, 0.0)},
                  {vec3f(0.1, 0.0, 0.0), vec3f(-0.1, 0.0, 0.0)}, player, cmd);
