...
class ConvexShape : public BoundingShape {
 ...
 protected:
  vec3f static support(const ConvexShape* shape0, const mat4f& mtx0,
                       const mat3f& iMtx0, const ConvexShape* shape1,
                       const mat4f& mtx1, const mat3f& iMtx1, vec3f dir);

  bool collides(const ConvexShape* other, const mat4f& thisMtx,
 ...
 public:
  ...
  ~ConvexShape() { free(self); }
  bool collides(const BoundingShape* other,
                const mat4f& thisMtx = mat4f::identity(),
                const mat4f& otherMtx = mat4f::identity()) const override;
  bool collides(const ConvexShape* other,
                const mat4f& thisMtx = mat4f::identity(),
                const mat4f& otherMtx = mat4f::identity()) const override;
  vec4f mtv(const BoundingShape* other,
            const mat4f& thisMtx = mat4f::identity(),
            const mat4f& otherMtx = mat4f::identity()) const override;
   ...
   * @return homogeneous vector of the furthest point of this shape in the
   * projected on to dir
   */
