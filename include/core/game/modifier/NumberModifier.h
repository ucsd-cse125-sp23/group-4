...
struct NumberModifierData : public ModifierData {
  Operation op;
  float val;
  NumberModifierData(Operation op, float val, unsigned long long duration = 0) : ModifierData(duration), op(op), val(val) {}
};
class NumberModifier : public Modifier {
 public:
  ...
  static float evaluate(float base, const std::vector<ModifierInstance*>& numberModifiers);
};