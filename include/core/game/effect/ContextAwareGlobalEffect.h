...
public:
	 ...
	 *					 status as the first PObject* in the vector parameter
	 * NOT_SELF_TAG_STATUS - target all players with a different it/notit
	 *						 status as the first PObject* in the vector parameter
	 * 
	 */
	enum class Targets {
		ALL, TAGGERS, NONTAGGERS, SELF, OTHER, SELF_TAG_STATUS, NOT_SELF_TAG_STATUS
	};
