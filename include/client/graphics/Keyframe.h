...
// The Keyframe struct, holds info for a single key within a channel and handles local computations.

enum class TangentRule {
    flat, line, smooth, fixed
};
...
struct Keyframe {
    ...
    void ComputeTangents(optional<Keyframe> prevOpt, optional<Keyframe> nextOpt)
    {
        ...
        switch (ruleIn) {
            ...
            case TangentRule::line:
                if (!prevOpt.exists) inCopyOut = true;
                else tangentIn = (value - prev.value) / (time - prev.time);
                break;
            case TangentRule::smooth:
                if (!prevOpt.exists) inCopyOut = true;
                else if (!nextOpt.exists) tangentIn = (value - prev.value) / (time - prev.time);
                else tangentIn = (next.value - prev.value) / (next.time - prev.time);
                break;
            default:
        ...
        switch (ruleOut) {
            case TangentRule::flat:
                ...
                break;
            case TangentRule::line:
                if (!nextOpt.exists) tangentOut = tangentIn;
                else tangentOut = (next.value - value) / (next.time - time);
                break;
            case TangentRule::smooth:
                if (!nextOpt.exists) tangentOut = tangentIn;
                else if (!prevOpt.exists) tangentOut = (next.value - value) / (next.time - time);
                else tangentOut = (next.value - prev.value) / (next.time - prev.time);
                break;
            default:
                break;
        }
    ...
    */
    void ComputeCoefficients(Keyframe next)
    {
        ...
        vec4 r = vec4(value, next.value, (next.time - time) * tangentOut, (next.time - time) * next.tangentIn);
