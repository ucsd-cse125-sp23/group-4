...
void PObjectCollection::addPObject(PObject* obj) { this->objs[obj->id] = obj; }
...
std::vector<size_t> PObjectCollection::getAllIds() {
  ...
  for (auto pair : this->objs) keys.push_back(pair.first);
  return keys;
}
void PObjectCollection::clear() { this->objs.clear(); }

PObject* PObjectCollection::operator[](size_t id) { return this->getById(id); }
