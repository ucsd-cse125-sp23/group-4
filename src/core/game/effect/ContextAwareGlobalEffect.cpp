...
void ContextAwareGlobalEffect::apply(Level* level, std::vector<PObject*> targets)
{
	...
	case ContextAwareGlobalEffect::Targets::SELF_TAG_STATUS:
		if (!targets[0]->hasModifier(TAGGED_STATUS_MODIFIER))
			return;
		data = static_cast<TaggedStatusModifierData*>(targets[0]->getModifiers(TAGGED_STATUS_MODIFIER)[0]->get());
	...
	for (auto pair : level->players)
	{
		data = static_cast<TaggedStatusModifierData*>(pair.second->getModifiers(TAGGED_STATUS_MODIFIER)[0]->get());
		if (data->isIt == targetTaggers)
			effect->apply(level, pair.second);
	}
}