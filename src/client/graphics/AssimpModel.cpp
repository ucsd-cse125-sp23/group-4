...
void AssimpModel::loadAssimpHelperNode(AssimpNode* node, glm::mat4 accTransform,
  ...
  for (unsigned int i = 0; i < aiNode->mNumChildren; i++) {
    ...
    loadAssimpHelperNode(childNode, node->accTransform, aiNode->mChildren[i],
                         scene);
...
void AssimpModel::loadAssimpHelperSkel(AssimpMesh* mesh, aiMesh* aiMesh,
  ...
  // mesh->vertices[vw.vertexInd].boneInds.x,
  // mesh->vertices[vw.vertexInd].boneInds.y,
  // mesh->vertices[vw.vertexInd].boneInds.z.
  // mesh->vertices[vw.vertexInd].boneInds.w);
}
