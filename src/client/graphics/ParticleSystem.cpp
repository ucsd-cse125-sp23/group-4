...
void ParticleSystem::Emit() {
	...
	Particle* p = new Particle(
		GetPosition() + DOFr::GetRandomVector3(initPosition) + (Tools::randomizeSphere() * radialPosition.GetValue()),
		DOFr::GetRandomVector3(initVelocity) + (Tools::randomizeSphere() * radialVelocity.GetValue()),
		initMass.GetValue());
	...
	p->ConfigureCollisions(elasticity.GetValue(), friction.GetValue(), &collision);
...
void ParticleSystem::Update(float deltaTime) {
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		bool keep = p->PostUpdate(deltaTime);	// integrate
...
void ParticleSystem::Show(const string name) {
	...
	TwAddVarRW(bar, "Draw Origin", TW_TYPE_BOOLCPP, &showBox, "true=SHOW false=HIDE");
	...
	// Emission
	TwAddVarRW(bar, "Rate", TW_TYPE_FLOAT, &creationRate, " min=0.0f max=50.0f step=1.0f group=Emission ");
	TwAddVarRW(bar, "Amount", TW_TYPE_UINT32, &creationAmount, " min=0 max=50 group=Emission ");
	TwAddButton(bar, "Emit Once", emit, this, " group=Emission ");
	...
	TwDefine(string(name + "/RandomOffset group=Transform").c_str());	// Move name/A into group Transform
	TwDefine(string(name + "/RandomOffset opened=false").c_str());	// Fold the group
